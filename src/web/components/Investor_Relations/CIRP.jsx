...
const Cirp = ({ releaseDataAPI, releaseData }) => {
  useEffect(() => {
    releaseDataAPI(5);
  }, []);
          ...
          <div className="row">
            <div className="col-md-12">
              ...
              <div className="pills">
                ...
                <Link to={WebRoutes.SHAREHOLDING_PATTERN}>
                  Shareholding Pattern
                </Link>
                <Link to={WebRoutes.CIRP} className="active">CIRP</Link>
              </div>
            </div>
          </div>
              ...
              releaseData.data.map((item, index) => (
                <div className="col-md-4" key={index}>
                  <div className="story">
                    ...
                    <p>
                      {item.file_title} & {item.invest_quater}
                    </p>
                  </div>
                </div>
              ))}
...
const mapDispatchToProps = (dispatch) => {
  return {
    releaseDataAPI: (data) => dispatch(releaseDataAPI(5)),
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(Cirp);
